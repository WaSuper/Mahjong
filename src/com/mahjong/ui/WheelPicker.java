...
import com.mahjong.tools.LinearGradient;
...
public class WheelPicker<T> extends View {
	...
	private com.mahjong.tools.LinearGradient mLinearGradient;
	...
	private Runnable mScrollerRunnable = new Runnable() {
		...
		public void run() {
			...
			if (mScroller.isFinished()
					|| (mScroller.getFinalY() == mScroller.getCurrY() && mScroller
							.getFinalX() == mScroller.getCurrX())) {
				...
				if (mCurrentPosition != position) {
					...
					mOnWheelChangeListener.onWheelSelected(
							mDataList.get(position), position);
				}
			}
		}
	};
	...
	private void initAttrs(Context context, @Nullable AttributeSet attrs) {
		...
		mItemMaximumWidthText = a
				.getString(R.styleable.WheelPicker_itemMaximumWidthText);
		...
		mCurrentPosition = a.getInteger(
				R.styleable.WheelPicker_currentItemPosition, 0);
		mItemWidthSpace = a.getDimensionPixelSize(
				R.styleable.WheelPicker_itemWidthSpace, getResources()
						.getDimensionPixelOffset(R.dimen.WheelItemWidthSpace));
		mItemHeightSpace = a.getDimensionPixelSize(
				R.styleable.WheelPicker_itemHeightSpace, getResources()
						.getDimensionPixelOffset(R.dimen.WheelItemHeightSpace));
		mIsZoomInSelectedItem = a.getBoolean(
				R.styleable.WheelPicker_zoomInSelectedItem, true);
		mIsShowCurtain = a.getBoolean(R.styleable.WheelPicker_wheelCurtain,
				true);
		mCurtainColor = a.getColor(R.styleable.WheelPicker_wheelCurtainColor,
				Color.parseColor("#303d3d3d"));
		mIsShowCurtainBorder = a.getBoolean(
				R.styleable.WheelPicker_wheelCurtainBorder, true);
		mCurtainBorderColor = a.getColor(
				R.styleable.WheelPicker_wheelCurtainBorderColor, Color.BLACK);
		mIndicatorText = a.getString(R.styleable.WheelPicker_indicatorText);
		mIndicatorTextColor = a.getColor(
				R.styleable.WheelPicker_indicatorTextColor,
				mSelectedItemTextColor);
		mIndicatorTextSize = a.getDimensionPixelSize(
				R.styleable.WheelPicker_indicatorTextSize, mTextSize);
		a.recycle();
	}
	...
	public void computeTextSize() {
		...
		mPaint.setTextSize(mSelectedItemTextSize > mTextSize ? mSelectedItemTextSize
				: mTextSize);
	...
	private void initPaint() {
		...
		mSelectedItemPaint = new Paint(Paint.ANTI_ALIAS_FLAG
	...
	private void computeFlingLimitY() {
		mMinFlingY = mIsCyclic ? Integer.MIN_VALUE : -mItemHeight
				* (mDataList.size() - 1);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mDrawnRect.set(getPaddingLeft(), getPaddingTop(), getWidth()
		...
		mFirstItemDrawY = (int) ((mItemHeight - (mSelectedItemPaint.ascent() + mSelectedItemPaint
				.descent())) / 2);
		// 中间的Item边框
		mSelectedItemRect.set(getPaddingLeft(), mItemHeight
				* mHalfVisibleItemCount, getWidth() - getPaddingRight(),
				mItemHeight + mItemHeight * mHalfVisibleItemCount);
		computeFlingLimitY();
		mCenterItemDrawnY = mFirstItemDrawY + mItemHeight
				* mHalfVisibleItemCount;
	...
	protected void onDraw(Canvas canvas) {
		...
		mPaint.setStyle(Paint.Style.FILL);
		// 首尾各多绘制一个用于缓冲
		for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1; drawDataPos <= drawnSelectedPos
				+ mHalfVisibleItemCount + 1; drawDataPos++) {
			...
			if (mIsTextGradual) {
				...
				if (distanceY < mItemHeight) { // 距离中心的高度小于一个ItemHeight才会开启渐变
					...
					mSelectedItemPaint.setColor(mLinearGradient
							.getColor(colorRatio));
			...
			String drawText = mDataFormat == null ? data.toString()
					: mDataFormat.format(data);
			// 在中间位置的Item作为被选中的。
			if (distanceY < mItemHeight / 2) {
				canvas.drawText(drawText, mFirstItemDrawX, itemDrawY,
						mSelectedItemPaint);
			} else {
				canvas.drawText(drawText, mFirstItemDrawX, itemDrawY,
						mTextPaint);
		...
		if (!TextUtils.isEmpty(mIndicatorText)) {
			canvas.drawText(mIndicatorText,
					mFirstItemDrawX + mTextMaxWidth / 2, mCenterItemDrawnY,
					mIndicatorPaint);
		}
	}
			...
			if (!mIsAbortScroller && mTouchDownY == mLastDownY) {
				...
				if (event.getY() > mSelectedItemRect.bottom) {
					int scrollItem = (int) (event.getY() - mSelectedItemRect.bottom)
							/ mItemHeight + 1;
					mScroller.startScroll(0, mScrollOffsetY, 0, -scrollItem
							* mItemHeight);

				} else if (event.getY() < mSelectedItemRect.top) {
					int scrollItem = (int) (mSelectedItemRect.top - event
							.getY()) / mItemHeight + 1;
					mScroller.startScroll(0, mScrollOffsetY, 0, scrollItem
							* mItemHeight);
				}
	...
	public synchronized void setCurrentPosition(int currentPosition,
		...
		if (smoothScroll && mItemHeight > 0) {
			...
			if (mOnWheelChangeListener != null) {
				mOnWheelChangeListener.onWheelSelected(
						mDataList.get(currentPosition), currentPosition);
			}
		}
	}
